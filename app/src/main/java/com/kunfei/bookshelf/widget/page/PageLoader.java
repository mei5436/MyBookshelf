...
//        Log.d(TAG, "skipPreChapter() called");
        if (mCurChapterPos <= 0) {
            return;
        }
...
//        Log.d(TAG, "openChapter() called with: pagePos = [" + pagePos + "]");
        mCurPagePos = pagePos;
...
//        Log.d(TAG, "parseCurChapter() called");
...
//        Log.d(TAG, "parsePrevChapter() called");
...
//        Log.d(TAG, "parseNextChapter() called");
        ...
        Single.create(new SingleOnSubscribe<TxtChapter>() {
            @Override
            public void subscribe(SingleEmitter<TxtChapter> e) throws Exception {
                ChapterProvider chapterProvider = new ChapterProvider(PageLoader.this);
                TxtChapter txtChapter = chapterProvider.dealLoadPageList(callback.getChapterList().get(nextChapterPos), mPageView.isPrepare());
                Log.d(TAG, "subscribe() called with: txtChapter = [" + txtChapter + "]");
                e.onSuccess(txtChapter);
            }
        })
                .compose(RxUtils::toSimpleSingle)
...
//        Log.d(TAG, "upTextChapter() called with: txtChapter ");
